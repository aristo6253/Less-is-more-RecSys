...
from googbooks import get_goodbooks_dataset
...
# TODO: change goodbooks to take a smaller number of users
# TODO: check how NDCG is calculated here

# load and process data
if args.data == 'goodbooks':
    data = get_goodbooks_dataset()
    train = data[0]
    test = data[1]
else:
...
test_ratings = [[] for i in range(test.shape[0])]
for i in range(len(test.data)):
    row = test.row[i]
    col = test.col[i]
    # print('oxiiiii:', test.data[(test.row == row) & (test.col == col)])
    test_labels[row].append(col)
    test_ratings[row].append(test.data[i])
print('Data loaded and processed.')

print('test labels:', test_labels[:5])
print('test ratings:', test_ratings[:5])
...
for batch in range(batch_no):
    ...
    #top@20
    recall_20, ndcg_20 = metrics(test_uids[start:end],predictions,20,test_labels, test_ratings)
    #top@40
    # recall_40, ndcg_40 = metrics(test_uids[start:end],predictions,40,test_labels, test_ratings)

    all_recall_20+=recall_20
    all_ndcg_20+=ndcg_20
    #all_recall_40+=recall_40
    #all_ndcg_40+=ndcg_40
    print('batch',batch,'recall@20',recall_20,'ndcg@20',ndcg_20)
print('-------------------------------------------')
print('recall@20',all_recall_20/batch_no,'ndcg@20',all_ndcg_20/batch_no)
