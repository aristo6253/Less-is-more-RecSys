...
class LightGCN(BasicModel):
    ...
    def bpr_loss(self, users, pos, neg):
        ...
        pos_ratings = torch.tensor(
            [
                self.dataset.UserItemNet[users[i].item(), pos[i].item()]
                for i in range(len(users))
            ]
        ).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
        # TODO: change the BPR loss below
        # loss = torch.mean(torch.nn.functional.softplus(neg_scores - (pos_ratings + pos_scores)))
        # loss = torch.mean(torch.nn.functional.softplus(neg_scores * pos_scores - pos_ratings))
        # loss = torch.mean(torch.nn.functional.softplus(neg_scores + pos_ratings - pos_scores))
        loss = torch.mean(torch.nn.functional.softplus(neg_scores - (pos_ratings * pos_scores)))
