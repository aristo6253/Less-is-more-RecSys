...
# TODO: the following two methods implement the weighted NDCG
...
def compute_ndcg(recommended_ratings, groundTrue_ratings, k):
    ideal_scores = sorted(groundTrue_ratings, reverse=True)
    if len(ideal_scores) > k:
        ideal_scores = ideal_scores[:k]
    ideal_dcg = compute_dcg(ideal_scores)
    actual_dcg = compute_dcg(recommended_ratings)
    if ideal_dcg == 0:
        return 0.0
