...
class Loader(BasicDataset):
    ...
    def __init__(self, config=world.config, path="../data/gowalla"):
        ...
        self.n_user = 0
        self.m_item = 0
        # TODO: load the correct files
        train_file = path + "/train-5000.txt"
        test_file = path + "/test-5000.txt"
        ratings_train_file = path + "/ratings-train-5000.txt"
        ratings_test_file = path + "/ratings-test-5000.txt"
        self.path = path
        ...
        # TODO: RUN WITH WEIGHTED ADJACENCY MATRIX
        with open(ratings_train_file) as r_file:
            array = np.fromstring(r_file.readline(), sep=",")

        self.UserItemNet = csr_matrix(
            (array, (self.trainUser, self.trainItem)), shape=(self.n_user, self.m_item)
        )

        # TODO: RUN WITH UNWEIGHTED ADJACENCY MATRIX
        """ self.UserItemNet = csr_matrix(
            (np.ones(len(self.trainUser)), (self.trainUser, self.trainItem)),
            shape=(self.n_user, self.m_item),
        ) """

        with open(ratings_test_file) as r_file_test:
            array_2 = np.fromstring(r_file_test.readline(), sep=",")

        self.UserItemNetTest = csr_matrix(
            (array_2, (self.testUser, self.testItem)), shape=(self.n_user, self.m_item)
        )
